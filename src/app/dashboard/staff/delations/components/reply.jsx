...
export function ReplyRevision({ revisionID, k, func }) {
    const { toast } = useToast();
    const formSchema = z.object({
        reply: z.string().max(100),
        veredit: z.string({
    ...
    async function onSubmit(data) {
        try {
            ...
            const res = await fetch('/api/staffs/reply-revision', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            if (res.status === 200) {
                toast({
                    title: "Revisão respondida com sucesso!",
                    description: "A revisão foi registrada e enviada! Você deve realizar as medidas cabiveis agora!"
                });
            } else {
                toast({
                    variant: "destructive",
